...
<body>
	<?php require_once __DIR__ .'/controllerEnderecoOcorrencia.php'; ?>
	...
	<div class="container">
		...
		<?php
			...
			$codp = $_GET['cod'];
			if(isset($codp)) {
				$result1 = $mysqli->query("SELECT ocorrencia_endereco_id FROM OCORRENCIA WHERE ocorrencia_id = $codp") or die($mysqli->error);
				$row1 = $result1->fetch_assoc();
				if(isset($row1['ocorrencia_endereco_id'])){
					$end_id = $row1['ocorrencia_endereco_id'];
				}

					$result = $mysqli->query("SELECT * FROM ENDERECO WHERE endereco_id = $end_id") or die($mysqli->error);	
			}
			else {
		...
		<br>
		<?php if(isset($codp)): ?>
			<h2 style="text-align: center">Ocorrência ID: <?php echo $codp;?></h2>
			<br>
		<?php endif; ?>	
	...
	<div class="row justify-content-center">
		...
		?>
				...
				<tr>
					...
					<td>
						<a href="enderecoOcorrencia.php?edit=<?php echo $row['endereco_id'] ?>" class = "btn btn-info">Edit</a>
						<a href="controllerEnderecoOcorrencia.php?delete=<?php echo $row['endereco_id']; ?>" class="btn btn-danger">Delete</a>
					</td>
				</tr>
		<?php endwhile; ?>
		</table>
	...
	<div class="row justify-content-center">
		<form action="controllerEnderecoOcorrencia	.php" method="POST">
